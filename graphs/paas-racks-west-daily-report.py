...
def failure_rates_by_zone(bool_prettytable=False):
    ...
    where tp.environ_name = 'paas-racks-west'
...
def failure_type_counts(bool_prettytable=False):
    ...
    sql_query = """
        ...
        where error_type != ''
              and tp.environ_name = 'paas-racks-west'
              and tp.time_started > DATE_SUB(NOW(), INTERVAL 90 day)
...
print '*********************************************************'
print '*** PaaS Racks West - Failure Rate By Day and Zone ***'
print '*********************************************************'
...
print ''
print '*******************************************************************************'
print '*** PaaS Racks West - Failure Counts - 7 Day - By Hour Method and Concurrency Count'
print '*******************************************************************************'
...
print ''
print '*****************************************************************'
print '*** PaaS Racks West - Failure By Zone and Concurrency Count ***'
print '*****************************************************************'
...
print ''
print '*****************************************************************'
print '*** PaaS Racks West - Instance IPs Not Pingable (after 10 minutes) - Last 7 Days ***'
print '*****************************************************************'
...
for index, row in enumerate(sql_result_data):
    ...
    if row[1] == 'paas-racks-west' and row[2] == 'dbaas-aw2az1-v1':
        az1_data.append([date_object, int(row[5] * 100)])
    elif row[1] == 'paas-racks-west' and row[2] == 'dbaas-aw2az2-v1':
        az2_data.append([date_object, int(row[5] * 100)])
    elif row[1] == 'paas-racks-west' and row[2] == 'dbaas-aw2az3-v1':
...
chart.title = 'PaaS Racks West AW2 - % Failure Rate by Hour and Zone - Last 30 Days'
...
chart.add('dbaas-aw2az1-v1', az1_data)
chart.add('dbaas-aw2az2-v1', az2_data)
chart.add('dbaas-aw2az3-v1', az3_data)

chart.render_to_file('paas-racks-west-failure-rate-by-hour.svg')
chart.render_to_png('paas-racks-west-failure-rate-by-hour.png')
...
chart.title = 'PaaS Racks West AW2 - % Failure Rate by Day - Last 90 Days'

chart.add('dbaas-aw2az1-v1', az1_data)
chart.add('dbaas-aw2az2-v1', az2_data)
chart.add('dbaas-aw2az3-v1', az3_data)

chart.render_to_file('paas-racks-west-failure-rate-by-day.svg')
chart.render_to_png('paas-racks-west-failure-rate-by-day.png')
